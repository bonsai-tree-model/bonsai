...
# fit the model
# max_rules=10, max_trees=3, random_state=42,
model = FIGSClassifier(max_rules=10, max_trees=3, random_state=42)  # initialize a model
model.max_depth = 3
# model.set_params(max_depth=3)
model.fit(X_train, y_train)   # fit model

# visualize the model
model.plot(feature_names=X_train.columns, filename='out.svg', dpi=300,impurity=True)
